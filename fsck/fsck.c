...
int main(int argc, char * const argv[])
{
	...
	ret = exfat_boot_region_check(&bd, &bs,
				      ...
				      true : false);
	if (ret) {
		exfat_err("failed to exfat_boot_region_check.\n");
		goto err;
	}
	...
	if (!exfat_fsck.exfat) {
		ret = -ENOMEM;
		exfat_err("failed to exfat_alloc_exfat.\n");
		goto err;
	}
